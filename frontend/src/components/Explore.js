...
const Explore = () => {
  ...
  const location = useLocation(); // Dashboard'dan gelen verileri yakalamak için

  // --- DASHBOARD BAĞLANTILI YENİ STATE BAŞLANGIÇLARI ---
  // Dashboard arama çubuğundan gelen bir kelime varsa onu al, yoksa boş string başla
  const [searchTerm, setSearchTerm] = useState(location.state?.initialSearch || "");
  
  // Eğer ana sayfadan bir kategoriye tıklanıp gelindiyse onu seç, yoksa varsayılan "All" yap:
  const [selectedCategory, setSelectedCategory] = useState(location.state?.category || "All");

  useEffect(() => {
    // Backend'den tüm koleksiyonu çekiyoruz (Seda'nın ayağa kaldıracağı backend adresi)
  ...
  // --- SQL SEED VERİLERİYLE TAM UYUMLU FİLTRELEME MANTIĞI ---
  const filteredItems = Array.isArray(items) 
    ? items.filter(item => {
        // 1. Arama filtresi (Hem başlıkta hem sanatçı adında arar)
  ...
  // Kategorilere göre dinamik küçük ikon seçici yardımcı fonksiyon
        ...
        <div className="flex gap-4">
          <div className="bg-white/70 border border-[#8e7eb5]/10 rounded-2xl px-5 py-3 flex items-center w-80 shadow-sm">
            ...
            <input 
              ...
              value={searchTerm} // Dashboard'dan gelen kelimeyi buraya sabitledik knk
          ...
          >
            ...
            <div className="mt-4 flex justify-between items-center">
              <span className="text-[#d4a373] font-bold text-sm">
                {item.price && parseFloat(item.price) > 0 ? `${parseInt(item.price)} TL` : "Not for Sale"}
              </span>
